"use client";
import { ViewsAnalyticsChartProps } from "./ViewsAnalyticsChart.types";
import * as React from "react";
import { TrendingUp } from "lucide-react";
import { Label, Pie, PieChart } from "recharts";

import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import {
  ChartConfig,
  ChartContainer,
  ChartTooltip,
  ChartTooltipContent,
} from "@/components/ui/chart";

export function ViewsAnalyticsChart(props: ViewsAnalyticsChartProps) {
  const { unique, repeated } = props;

  const chartData = React.useMemo(
    () => [
      { browser: "Unicas", visitors: unique, fill: "var(--color-chrome)" },
      { browser: "Repetidas", visitors: repeated, fill: "var(--color-safari)" },
    ],
    [unique, repeated]
  ); // ✅ Se actualiza solo cuando 'unique' o 'repeated' cambian.

  const chartConfig = {
    visitors: {
      label: "COntraseñas totales",
    },
    chrome: {
      label: "Contraseñas unicas",
      color: "#2563eb",
    },
    safari: {
      label: "Contraseñas repetidas",
      color: "#60a5fa",
    },
    firefox: {
      label: "Firefox",
      color: "#a6341b",
    },
    edge: {
      label: "Edge",
      color: "#099921",
    },
    other: {
      label: "Other",
      color: "#2d1c72",
    },
  } satisfies ChartConfig;

  const totalVisitors = React.useMemo(() => {
    return chartData.reduce((acc, curr) => acc + curr.visitors, 0);
  }, [chartData]);

  return (
    <Card className="flex flex-col">
      <CardHeader className="items-center pb-0">
        <CardTitle>Contraseñas</CardTitle>
        <CardDescription>Repetidas vs Unicas</CardDescription>
      </CardHeader>
      <CardContent className="flex-1 pb-0">
        <ChartContainer
          config={chartConfig}
          className="mx-auto aspect-square max-h-[250px]"
        >
          <PieChart>
            <ChartTooltip
              cursor={false}
              content={<ChartTooltipContent hideLabel />}
            />
            <Pie
              data={chartData}
              dataKey="visitors"
              nameKey="browser"
              innerRadius={60}
              strokeWidth={5}
            >
              <Label
                content={({ viewBox }) => {
                  if (
                    typeof viewBox === "object" &&
                    viewBox !== null &&
                    "cx" in viewBox &&
                    "cy" in viewBox
                  ) {
                    return (
                      <text
                        x={viewBox.cx}
                        y={viewBox.cy}
                        textAnchor="middle"
                        dominantBaseline="middle"
                      >
                        <tspan
                          x={viewBox.cx}
                          y={viewBox.cy ?? 0}
                          className="fill-foreground text-3xl font-bold"
                        >
                          {totalVisitors.toLocaleString()}
                        </tspan>
                        <tspan
                          x={viewBox.cx}
                          y={(viewBox.cy ?? 0) + 24}
                          className="fill-muted-foreground"
                        >
                          Creadas
                        </tspan>
                      </text>
                    );
                  }
                }}
              />
            </Pie>
          </PieChart>
        </ChartContainer>
      </CardContent>
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 font-medium leading-none">
          Trata de no tener las mismas contraseñas{" "}
          <TrendingUp className="h-4 w-4" />
        </div>
        <div className="leading-none text-muted-foreground">
          Mostrando el total de las contraseñas
        </div>
      </CardFooter>
    </Card>
  );
}
